...
class Oregional extends CI_Controller{
        ...
        <div id="tabs-'.$row['dep_id'].'">
          <div class="row">
            <div class="'.$estil.'" role="alert">
              ...
              if($this->dep_id==$row['dep_id'] || $this->fun_id==399 || $this->fun_id==401 || $this->fun_id==413 || $this->fun_id==418 || $this->fun_id==1259){
                $tabla.='
              ...
              $tabla.='
              <a href="javascript:abreVentana(\''.site_url("").'/rep_oregional/'.$row['dep_id'].'\');" title="IMPRIMIR OPERACIONES REGIONAL" class="btn btn-default">
                <img src="'.base_url().'assets/Iconos/printer_empty.png" WIDTH="25" HEIGHT="25"/>&nbsp; REP. OPERACIÓN (FORM N° 2)
              </a>
              <a href="'.site_url("").'/me/exportar_oregional/'.$row['dep_id'].'" title="EXPORTAR OPERACIONES" target=_blank class="btn btn-warning"><img src="'.base_url().'assets/Iconos/application_form_add.png" WIDTH="25" HEIGHT="25"/>&nbsp; EXPORTAR OPERACIONES</a>
                ...
                <table class="table table-bordered">
                  <thead>
                    <tr>
                      ...
                      <th style="width:5%;">PRIORIDAD_2</th>
              ...
              foreach($oregional as $row_or){
                $uresponsables=$this->model_objetivoregion->get_urep_oregional($row_or['or_id']);
                $nro++;
                $tabla.='<tr>';
                  $tabla.='
                  ...
                  <td>';
                  if($this->dep_id==$row['dep_id'] || $this->fun_id==399 || $this->fun_id==401 || $this->fun_id==418 || $this->fun_id==413 || $this->fun_id==1259){
                  ...
                  <td align=length title='.count($uresponsables).'>';
                    $uresp='';$sw=0;
                    foreach($uresponsables as $row_ue){
                      $uresp=$uresp.' '.$row_ue['tipo'].' '.$row_ue['proy_nombre'].' '.$row_ue['abrev'].' - ';
                    }
                  $tabla.=''.$uresp.'
                  </td>
                            ...
                            if(count($nivel)!=0){
                              $tabla.='
                                <table class="table table-bordered" style="width:100%;">
                                  <thead>
                                    <tr>
                                      <th style="width:100%;"> '.$row['dist_id'].' - '.$rown['descripcion'].'</th>
                                    <tr>
                                  </thead>
                                  <tbody>
                                  <tr>
  ...
  public function rep_lista_form2_sp($dep_id){ 
    ...
    <table cellpadding="0" cellspacing="0" class="tabla" border=0.1 style="width:100%;" align=center>
      <thead>
        <tr style="font-size: 6.7px;" bgcolor="#eceaea" align=center>
          ...
          <th style="width:25;">UNIDAD RESPONSABLE</th>
    ...
    foreach($lista_ogestion as $row){
      $uresponsables=$this->model_objetivoregion->get_urep_oregional($row['or_id']);
      $meta='';
      if($row['indi_id']==2){
        $meta='%';
      }
      ...
      <tr style="font-size: 6.5px;" bgcolor='.$color.'>
        ...
        <td style="width:12%;">'.$row['or_verificacion'].'</td>
      </tr>';
    }
    $tabla.='
      </tbody>
  ...
  //// Lista de OPeraciones Regional form 2 SIN PRESUPUESTO
  public function exportar_lista_form2_sp($dep_id){ 
    $tabla='';
    ...
    <table border="1" cellpadding="0" cellspacing="0" class="tabla" style="width:100%;">
      <thead>
        <tr>
          <td colspan="23" style="font-size: 18px; height:20px;"><b>OPERACIONES '.$this->gestion.' - '.strtoupper($regional[0]['dep_departamento']).'</b></td>
        </tr>
        <tr style="font-size: 13px;" bgcolor="#eceaea" align=center>
          <th style="width:1%;height:50px;">#</th>
          <th style="width:2%;"><b>COD. OE.</b></th>
          <th style="width:2%;"><b>COD. ACP.</b></th>
          <th style="width:2%;"><b>COD. OPE.</b></th>
          <th style="width:15%;">OPERACI&Oacute;N REGIONAL '.$this->gestion.'</th>
          ...
          <th style="width:12%;">RESULTADO</th>
          <th style="width:12%;">INDICADOR</th>
          <th style="width:30%;">UNIDAD RESPONSABLE</th>
          <th style="width:4%;">META</th>
          
          <th style="width:10%;">ENE.</th>
          ...
          <th style="width:10%;">AGO.</th>
          <th style="width:10%;">SEPT.</th>
          <th style="width:10%;">OCT.</th>
          <th style="width:10%;">NOV.</th>
          <th style="width:10%;">DIC.</th>
          <th style="width:12%;">MEDIO DE VERIFICACI&Oacute;N</th>
        </tr>
      </thead>
      <tbody>';
    $nro=0;$monto_total=0;
    foreach($lista_ogestion as $row){
      $uresponsables=$this->model_objetivoregion->get_urep_oregional($row['or_id']); // unidades responsables
      $temporalidad_x_actividades=$this->model_objetivoregion->get_temp_or_x_actividades($row['or_id']); // temporalidad
      ...
      $tabla.='
      <tr bgcolor='.$color.'>
        <td style="width:1%; height:60px;" align=center>'.$nro.'</td>
        <td style="width:2%; font-size: 12px;" align="center"><b>'.$row['obj_codigo'].'</b></td>
        <td style="width:2%; font-size: 12px;" align="center"><b>'.$row['og_codigo'].'</b></td>
        <td style="width:2%; font-size: 12px;" align="center"><b>'.$row['or_codigo'].'</b></td>
        <td style="width:15%;">'.mb_convert_encoding(strtoupper($row['or_objetivo']), 'cp1252', 'UTF-8').'</td>
        <td style="width:12%;">'.mb_convert_encoding(strtoupper($row['or_producto']), 'cp1252', 'UTF-8').'</td>
        <td style="width:12%;">'.mb_convert_encoding(strtoupper($row['or_resultado']), 'cp1252', 'UTF-8').'</td>
        <td style="width:12%;">'.mb_convert_encoding(strtoupper($row['or_indicador']), 'cp1252', 'UTF-8').'</td>
        <td style="width:30%;">';
          $uresp='';
          foreach($uresponsables as $row_ue){
            $uresp=$uresp.' '.$row_ue['tipo'].' '.$row_ue['proy_nombre'].' '.$row_ue['abrev'].' - ';
          }
        $tabla.=''.mb_convert_encoding(strtoupper($uresp), 'cp1252', 'UTF-8').'
        </td>
        <td style="width:4%; font-size: 13px;"><b>'.round($row['or_meta'],2).''.$meta.'</b></td>';

        if(count($temporalidad_x_actividades)!=0){
          for ($i=1; $i <=12 ; $i++) { 
            $tabla.='<td style="width:10%;" >'.round($temporalidad_x_actividades[0]['m'.$i]).'</td>';
          }
        }
        else{
          for ($i=1; $i <=12 ; $i++) { 
            $tabla.='<td style="width:10%;bgcolor:red;">-</td>';
          }
        }
        $tabla.='
        <td style="width:12%;">'.mb_convert_encoding(strtoupper($row['or_verificacion']), 'cp1252', 'UTF-8').'</td>
      </tr>';
    }
    $tabla.='
      </tbody>
