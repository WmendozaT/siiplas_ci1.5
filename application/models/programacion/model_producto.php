...
class model_producto extends CI_Model {
    ...
    /*--- VERIF ALINEACION DEL REQUERIMIENTO A LA ACTIVIDAD 2027 ---*/
    ...
    /*------- GET UNIDAD RESPONSABLE para Actividades de los Programas Bolsas-------*/
    function get_unidadResponsable_programaBolsa($uni_resp){
        $sql = 'SELECT * 
                from vista_ver_uresp_proyecto
                where com_id='.$uni_resp.''; 
        $query = $this->db->query($sql);
        return $query->result_array();
    }
...
/*    function get_uni_resp_prog770($com_id,$uni_resp){
    ...
    function get_lista_form4_uresp_consolidado($com_id){
        ...
        -- Lógica de Unidades Responsables (Bolsas)(uresp.tipo_subactividad, \' \', uresp.com_componente, \' - \', uresp.proy_nombre, \' [\', uresp.abrev, \']\')
        CASE 
            WHEN p.uni_resp = ".$com_id." THEN CONCAT(uresp.tipo_subactividad, ' ', uresp.com_componente, ' - ',uresp.proy_nombre, ' [',uresp.abrev,']')
        ...
        SELECT 
            ...
            CASE 
                ...
                WHEN uresp.com_id IS NOT NULL THEN 
                    CONCAT(uresp.tipo_subactividad, ' ', uresp.com_componente,' - ',uresp.proy_nombre, ' [',uresp.abrev,']' )
                ELSE 
                    p.prod_unidades 
            END AS prod_unidades,
