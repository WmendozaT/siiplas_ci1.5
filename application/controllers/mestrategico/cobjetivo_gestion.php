...
class Cobjetivo_gestion extends CI_Controller {
  ...
  public function __construct (){
    ...
    public function list_objetivos_gestion(){
      ...
      $data['modal_migracion']=$this->modal_migracion_form2(); /// MODAL DE MIGRACION OPERACIONES
      $data['modal_migracion_alineacion_form4']=$this->modal_migracion_alineacion_form4(); /// MODAL DE MIGRACION PARA ALINEAR LAS ACTIVIDADES

      $this->load->view('admin/mestrategico/objetivos_gestion/list_ogestion_general', $data);
    }

    /// modal de migracion Operaciones
    public function modal_migracion_form2(){
      $tabla='';
        $tabla.= '
            ...
            <div class="modal-dialog" id="dialog_subirr">
                <div class="modal-content" style="border-radius: 4px; box-shadow: 0 8px 30px rgba(0,0,0,0.3); border: none; overflow: hidden;">
                    ...
                    <div class="modal-body" style="padding: 25px; background: #ffffff;">
                        ...
                        <form action="' . site_url('mestrategico/cobjetivo_regional/valida_add_operaciones_regionales') . '" method="post" enctype="multipart/form-data" id="form_subir_form2" autocomplete="off" style="padding:0; background:transparent;">
    ...
    /// modal de migracion Alienacion de Actividades form 4 a Operaciones Form 2
    public function modal_migracion_alineacion_form4(){
      $tabla='';
        $tabla.= '
        <div class="modal fade" id="modal_importar_alineacion" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-hidden="true" role="dialog">
            <div class="modal-dialog" id="dialog_subirr">
                <div class="modal-content" style="border-radius: 4px; box-shadow: 0 8px 30px rgba(0,0,0,0.3); border: none; overflow: hidden;">
                    <!-- CABECERA DEL COMPONENTE -->
                    <div class="modal-header" style="background: #f8fafc; border-bottom: 1px solid #e2e8f0; padding: 15px 20px;">
                        ...
                        <h4 class="modal-title" style="font-weight: bold; color: #1e293b; font-size: 13.5px; text-transform: uppercase; letter-spacing: 0.3px;">
                            <i class="fa fa-upload text-primary"></i> Importar Actividades para alineación a Operaciones
                        </h4>
                    </div>
                    ...
                    <div class="modal-body" style="padding: 25px; background: #ffffff;">
                        ...
                        <form action="' . site_url('programacion/producto/valida_alineacion_a_form2') . '" method="post" enctype="multipart/form-data" id="form_subir_alineacion" autocomplete="off" style="padding:0; background:transparent;">
                            ...
                            <div id="mensaje_alineacion" style="margin: 10px 0; font-size: 11px;"></div>

                            <!-- Botón de Envío y Validación Masiva -->
                            <div style="margin-top: 25px;">
                                <button type="button" id="btn_subir_alineacion" class="btn btn-success btn-block" style="font-weight: bold; border-radius: 3px; padding: 8px 16px; font-size: 13px; background: #2e7d32; border-color: #2e7d32; text-transform: uppercase; letter-spacing: 0.3px;">
                                    <i class="fa fa-check-circle"></i> VALIDAR Y SUBIR ARCHIVO
                                </button>
                            </div>

                            <!-- Animación Pre-Loader de la Planilla -->
                            <div id="loads_alineacion" class="text-center" style="display: none; margin-top: 20px; padding: 10px; border: 1px dashed #2e7d32; background: #f0fdf4; border-radius: 4px;">
                                <i class="fa fa-refresh fa-spin fa-2x text-success" style="margin-bottom: 5px;"></i>
                                <p style="margin: 0; font-size: 11.5px; color: #16a34a;"><b>Sincronizando celdas, por favor espere...</b></p>
                            </div>
                        </form>
  ...
  public function rep_alineacion_acp_act($og_id){
    ...
    $data['titulo']='
        ...
        if($this->fun_id==399 || $this->fun_id==401  || $this->fun_id==418 || $this->fun_id==413 || $this->fun_id==1259 || $this->fun_id==1301){
