...
class Cajustes extends CI_Controller {
    ...
    public function menu_ajustes2(){
      $api_url = 'http://planificacion.cns.gob.bo:8081/index.php/mnt/presentacion_poa';
        ...
        // $api_url = 'dominio-del-primer-sistema.com?key=clave_secreta_sitio_A';
...
//phpinfo();
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); // Devolver la respuesta como cadena, no imprimirla directamente
        ...
        if (curl_errno($ch)) {
            ...
            curl_close($ch);
            log_message('error', 'Error al consumir API: ' . $error_msg);
            echo "Error al conectar con la API: " . $error_msg;
            return;
        }
        ...
        // --- Usar los datos en la Vista ---
        
        // Preparamos los datos para pasarlos a una vista de CodeIgniter
        $data['titulo'] = "Usuarios obtenidos de la API externa";
        $data['usuarios'] = $users;

        $this->load->view('admin/mantenimiento/ajustes_siiplas/menu_ajustes', $data);
        // Cargamos una vista para mostrar la tabla
    ...
    public function menu_ajustes(){
      ...
      $data['certificado']='';
/*      $listado=$this->model_configuracion->get_list_usuarios_desabilitados();
      foreach ($listado as $row){
        //echo $row['fun_id']."-".$row['fun_nombre']."<br>";
        $this->db->where('fun_id', $row['fun_id']);
            ...
            if($i != 0){ 
              $datos = explode(";",$linea);
                ...
                if(count($datos)==23){
                  ...
                  $get_informacion_alineacion=$this->model_objetivogestion->get_alineacion_habilitado_oregional_a_form4_ajuste($codigo_og,$codigo_or,$dep_id);
