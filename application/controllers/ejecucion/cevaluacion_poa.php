<?php
class Cevaluacion_poa extends CI_Controller {
  ...
  public function __construct (){
    ...
    /*----- formulario Evaluacion POA ------*/
    public function formulario_evaluacionpoa($com_id){
      ...
      if(count($data['componente'])!=0){
          ...
          <h1><small>SUBACTIVIDAD : </small> '.$data['componente'][0]['tipo_subactividad'].' '.$data['componente'][0]['serv_cod'].' '.$data['componente'][0]['serv_descripcion'].'</h1>';
          ...
          $this->load->view('admin/evaluacion/evaluacion_poa/mis_productos', $data);
      ...
      $tabla.=' <div class="table-responsive">
                  <table id="dt_basic" class="table table table-bordered" width="100%">
                      ...
                      foreach($productos as $rowp){
                        $diferencia=$this->verif_trimestre_no_ejecutado($rowp['prod_id']); /// Saldo de Trimestres Anteriores
                        if($diferencia[1]!=0 || $diferencia[2]!=0){
/*                          $programado=$this->model_evaluacion->programado_trimestral_productos($this->tmes,$rowp['prod_id']); //7 Meta programado Trimestre
                          $prog_trimestre=0;
                          if(count($programado)!=0){
                            $prog_trimestre=$programado[0]['trimestre'];
                          }

                          $ejecutado=$this->model_evaluacion->ejecutado_trimestral_productos($this->tmes,$rowp['prod_id']); //7 Meta programado Trimestre
                          $ejec_trimestre=0;
                          if(count($ejecutado)!=0){
                            $ejec_trimestre=$ejecutado[0]['trimestre'];
                          }*/

                          $nro++;
                          $tabla .='
                            <tr>
                              <td align=center title='.$rowp['prod_id'].'>'.$nro.'</td>
                              ...
                              <td align=right>'.round($rowp['prod_linea_base'],2).'</td>
                              <td align=right>'.round($rowp['prod_meta'],2).'</td>
                              <td>'.$rowp['prod_fuente_verificacion'].'</td>
                              <td align=right><b>'.$diferencia[1].'</b></td>
                              <td align=right bgcolor="#daf3da"><b>'.$diferencia[2].'</b></td>
                              <td align=right bgcolor="#fbf2f2"><b>'.$diferencia[3].'</b></td>
                              <td></td>
                              <td></td>
                            </tr>';
                        }
    ...
    /*---VERIFICANDO VALORES NO EJECUTADOS EN TRIMESTRES ANTERIORES --*/
    function verif_trimestre_no_ejecutado($prod_id){
      $suma_trimestre_programado=0;
      $suma_trimestre_ejecutado=0;

      $meta_trimestre[1]=0; /// Programado
      $meta_trimestre[2]=0; /// Ejecutado
      $meta_trimestre[3]=0; /// Diferencia

      if($this->tmes==1){ /// si es primer trimestre
        ...
        if(count($trimestre_prog)!=0){
          $suma_trimestre_programado=$suma_trimestre_programado+$trimestre_prog[0]['trimestre'];
        }


        $trimestre_ejec = $this->model_evaluacion->ejecutado_trimestral_productos($this->tmes,$prod_id); /// Trimestre Ejecutado
        if(count($trimestre_ejec)!=0){
          $suma_trimestre_ejecutado=$suma_trimestre_ejecutado+$trimestre_ejec[0]['trimestre'];
        }

        $meta_trimestre[3]=0; /// Diferencia
      }
      else{ /// Apartir del segundo trimestre
        for ($i=1; $i <=$this->tmes-1; $i++) { 
          $trimestre_prog = $this->model_evaluacion->programado_trimestral_productos($i,$prod_id); /// Trimestre Programado
          if(count($trimestre_prog)!=0){
            $suma_trimestre_programado=$suma_trimestre_programado+$trimestre_prog[0]['trimestre'];
          }


          $trimestre_ejec = $this->model_evaluacion->ejecutado_trimestral_productos($i,$prod_id); /// Trimestre Ejecutado
          if(count($trimestre_ejec)!=0){
            $suma_trimestre_ejecutado=$suma_trimestre_ejecutado+$trimestre_ejec[0]['trimestre'];
          }
        }

        $meta_trimestre[3]=($suma_trimestre_programado-$suma_trimestre_ejecutado); /// Diferencia
      }


      /// -------------------------------
      $meta_trimestre[1]=$suma_trimestre_programado; /// Programado
      $meta_trimestre[2]=$suma_trimestre_ejecutado; /// Ejecutado
